...
void MiniImpedanceNode::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  ...
  if (msg->position.size() < 2 || msg->velocity.size() < 2) {
    RCLCPP_WARN(this->get_logger(), "JointState message has insufficient data (position or velocity < 2)");
    return;
  }
...
void MiniImpedanceNode::update()
{
  ...
  pos_(0) = l0 + l1 * std::cos(q_[0]) + l2 * std::cos(q_[0] + q_[1]); // X
  pos_(1) = (l1 * std::sin(q_[0]) + l2 * std::sin(q_[0] + q_[1])); // Y
  pos_(2) = 0.0033;                                              // Z
  

  // Analytical Jacobian
  Eigen::MatrixXd jacobian(2, 2);
  jacobian(0, 0) = -l1 * std::sin(q_[0]) - l2 * std::sin(q_[0] + q_[1]);
  jacobian(0, 1) = -l2 * std::sin(q_[0] + q_[1]);
  jacobian(1, 0) = l1 * std::cos(q_[0]) + l2 * std::cos(q_[0] + q_[1]);
  jacobian(1, 1) = l2 * std::cos(q_[0] + q_[1]);
