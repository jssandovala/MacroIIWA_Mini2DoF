...
def pub_Pose():
    ...
    publisher = node.create_publisher( PoseStamped,'/mini/pose_sim', 10)
...
def wait_for_initial_pose():
    global marker_pose
    _, smg = wait_for_message(JointState, node,'/joint_states')
    q_int = np.array(smg.position)
    l0 = 0.043 + 0.084
    l1 = 0.2
    l2 = 0.2
    marker = PoseStamped()
    marker.pose.position.y = l1 * math.sin(q_int[0]) + l2 * math.sin(q_int[0] + q_int[1]) #X
    marker.pose.position.z = 0.0033+0.1
    marker.pose.position.x = l0 + l1 * math.cos(q_int[0]) + l2 * math.cos(q_int[0] + q_int[1]) #Z*/
...
def make2DofMarker(marker_pose, scale=0.1):
    ...
    int_marker.name = 'mini_TCP_2D_sim'
    int_marker.description = 'mini_TCP_sim'
...
if __name__ == '__main__':
    ...
    node = rclpy.create_node('mini_pose_sim')
    br = TransformBroadcaster(node)
    node.get_logger().info('le node est lancee')

    server = InteractiveMarkerServer(node, 'mini_pose_sim')
