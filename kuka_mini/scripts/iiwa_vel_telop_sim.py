...
class iiwa_Vel(Node):
    def __init__(self):
        ...
        # self.fk_mini_pin_sub = self.create_subscription(
        #     PoseStamped,
        #     '/fk_mini_pin',
        #     self.fk_mini_pin_sub_callback,
        #     10
        # )

        # self.fk_kuka_pin_sub = self.create_subscription(
        #     PoseStamped,
        #     '/fk_kuka',
        #     self.fk_kuka_pin_sub_callback,
        #     10
        # )
        ...
        self.q_int = np.array(smg.position[2:9])
        self.q = self.q_int
        ...
        self.d_cart_pose.pose.position.x =-0.778
        self.d_cart_pose.pose.position.y = 0.0
        self.d_cart_pose.pose.position.z =0.427
        self.d_cart_pose.pose.orientation.x = 0.502 
        self.d_cart_pose.pose.orientation.y = 0.500 
        self.d_cart_pose.pose.orientation.z = -0.483 
        self.d_cart_pose.pose.orientation.w = -0.514 

        self.get_logger().info('iiwa_vel Node has been started.')


    def joint_states_callback(self, msg: JointState):
        self.q = np.array(msg.position[2:9])
    ...
    # def fk_mini_pin_sub_callback(self, msg: PoseStamped):
    #     self.fk_mini_pin_info=msg

    # def fk_kuka_pin_sub_callback(self, msg: PoseStamped):
    #     self.fk_kuka_pin_sub_info=msg
    ...
    def Pos_error (self, pos_d ,pos):
        ...
        #self.get_logger().info(f'pos_d: x={pos_d.pose.position.x:.3f}, y={pos_d.pose.position.y:.3f}, z={pos_d.pose.position.z:.3f}')
        #self.get_logger().info(f'pos: x={pos.pose.position.x:.3f}, y={pos.pose.position.y:.3f}, z={pos.pose.position.z:.3f}')
        #self.get_logger().info(f'Position error: dx={dX[0]:.3f}, dy={dX[1]:.3f}, dz={dX[2]:.3f}')
        return(dX)
    ...
    def main_loop(self):
        while rclpy.ok():
            ...
            #self.get_logger().info(f'desired cartesian position kariiiiiii: x={self.d_cart_pose.pose.position.x:.3f}, y={self.d_cart_pose.pose.position.y:.3f}, z={self.d_cart_pose.pose.position.z:.3f}')

            #Check if it will use the original pose or it would move w.r.t the error from the mini
            if self.initial_pose_mode:
                dX_c = self.Pos_error(self.d_cart_pose,self.cart_pose)
                # self.get_logger().info("Kuka is staying at its initial pose.")
                print(f"Kuka is staying at its initial pose.", end='\r')

               

            else:
                if len(self.fk_mini_info.data) >= 2:
                    ref_mini = np.array([0, 0.225]) #14cm now offset of 0.14+0.085 for now because of the real robot
            ...
            dq= np.dot(J_p,(1.5*dX_c))

            # null space
            dq_N1= np.dot( (np.eye(7) - np.dot(J_p,J)), (self.q_null - self.q))
            #self.get_logger().info(f'q_null : {self.q_null}')
