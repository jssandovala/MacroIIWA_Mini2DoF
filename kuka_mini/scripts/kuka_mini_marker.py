...
def pub_Pose():
    ...
    publisher = node.create_publisher( PoseStamped, '/kuka_mini/pose', 10)
...
def iiwa_mini_robot():
        ...
        L = [
            ...
            RevoluteDH(a=0.2, d=0.0,   alpha=0,                      qlim=np.array([max_positions[8], max_positions[8]])) 
...
def wait_for_initial_pose():
    ...
    _, smg2 = wait_for_message(JointState, node,'RR/joint_states') 

    q_int = [smg.position[0], smg.position[1], smg.position[2], smg.position[3], smg.position[4], smg.position[5], smg.position[6], smg2.position[0], smg2.position[1]]
