...
class iiwa_Vel(Node):
    def __init__(self):
        ...
        self.d_cart_pose = self.target_pose(self.q_int, self.error_mini) #To make the initial pose of the robot be the current one
    ...
    def error_mini_callback(self, msg: Float64MultiArray):
        self.error_mini[0] = msg.data[0]
        self.error_mini[1] = msg.data[1]
        # self.save_data = False

        if np.linalg.norm(self.error_mini) < 0.01:
            self.error_mini[0] = 0.0
            self.error_mini[1] = 0.0
            # self.save_data = True
        
        # print(self.error_mini[0], self.error_mini[1])
    ...
    def target_pose(self,data_q, data_err):
        T=self.kuka.fkine(data_q)
        TT=np.array([[T.n[0],T.o[0],T.a[0],T.t[0]], [T.n[1],T.o[1],T.a[1],T.t[1]], [T.n[2],T.o[2],T.a[2],T.t[2]], [0, 0, 0, 1]])
        T_err=np.eye(4)
        T_err[0, 3] = -data_err[0]
        T_err[2, 3] = -data_err[1]
        T_target=np.dot(TT,T_err)
        quat = Quaternion(matrix=T_target)
        FK= PoseStamped()
        FK.pose.position.x = T_target[0,3]
        FK.pose.position.y = T_target[1,3]
        FK.pose.position.z = T_target[2,3]
        FK.pose.orientation.x = quat[1]
        FK.pose.orientation.y = quat[2]
        FK.pose.orientation.z = quat[3]
        FK.pose.orientation.w = quat[0]
    ...
    def main_loop(self):
        while rclpy.ok():
            ...
            self.d_cart_pose = self.target_pose(self.q, self.error_mini) #q instead of q_int?
            dX_c = self.Pos_error(self.d_cart_pose,self.cart_pose)

            # print(self.d_cart_pose)
            ...
            dq= np.dot(J_p,(1.3*dX_c)) #it was 6
