...
def Mm_pose_callback(msg: PoseStamped):
    global Mm_pose_
    Mm_pose_ = msg
...
def wait_for_initial_pose():
    global Mm_pose_
    while Mm_pose_ is None:
        rclpy.spin_once(node, timeout_sec=0.1) #Wait for initial pose

    marker = PoseStamped()
    marker.pose.position.x = Mm_pose_.pose.position.x
    marker.pose.position.y = Mm_pose_.pose.position.y
    marker.pose.position.z = Mm_pose_.pose.position.z
    marker.pose.orientation.x = Mm_pose_.pose.orientation.x
    marker.pose.orientation.y = Mm_pose_.pose.orientation.y
    marker.pose.orientation.z = Mm_pose_.pose.orientation.z
    marker.pose.orientation.w = Mm_pose_.pose.orientation.w

    # print("Initial pose received from /world_T_ee_IM")
    # print(f"Position: {marker.pose.position.x}, {marker.pose.position.y}, {marker.pose.position.z}")
    # print(f"Orientation: {marker.pose.orientation.x}, {marker.pose.orientation.y}, {marker.pose.orientation.z}, {marker.pose.orientation.w}")
...
if __name__ == '__main__':
    ...
    node.create_subscription(PoseStamped, 'world_T_ee_IM', Mm_pose_callback, 10) #This has to be before the wait_for_initial_pose
