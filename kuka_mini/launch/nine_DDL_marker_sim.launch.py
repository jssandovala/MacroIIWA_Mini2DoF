...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            'kuka_controller',
            default_value='iiwa_mini/velocity_controller', #velocity_controller , iiwa_arm_controller
            description='Robot controller to start.',
        )
    )
    ...
    rviz_config_file = PathJoinSubstitution(
        [FindPackageShare(description_package), 'rviz', 'kuka_mini_interactiveMarker_.rviz']
    )
    ...
    redundacy_ = Node(
        package='kuka_mini',
        executable='iiwa_mini2r_redundancy_sim.py',
        name='iiwa_mini2r_redundancy_sim',
        output='screen',
    )

    mini2r_impedance = Node(
        package='kuka_mini',
        executable='mini2r_joint_impedance_sim.py',
        name='mini2r_joint_impedance_sim',
        output='screen',
    )

    iiwa_speed = Node(
        package='kuka_mini',
        executable='iiwa_joint_speed_sim.py',
        name='iiwa_FK_tool_sim',
        output='screen',
    )

    interactive_marker_ = Node(
        package='kuka_mini',
    ...
    """delay_interactive_marker_after_mini_effort_tm_sim = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=mini_effort_tm_sim,
            on_exit=[interactive_marker_],
        )
    )"""
