...
MiniEffortNode::MiniEffortNode() : Node("mini_effort_node")
{
  ...
  // desired_pose_pub = this->create_publisher<std_msgs::msg::Float64MultiArray>(
  //   "/desired_pose_wrt_mini", 10);

  // error_graph_pub = this->create_publisher<std_msgs::msg::Float64MultiArray>(
  //   "/error_mini_graph", 10);
  ...
  pedal_sub = this->create_subscription<sensor_msgs::msg::Joy>(
    "/joy", 10,
    std::bind(&MiniEffortNode::pedal_callback, this, std::placeholders::_1));
...
void MiniEffortNode::pedal_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
{
  // Lock the mutex to ensure thread safety
  std::lock_guard<std::mutex> lock(data_mutex_);
  ...
  // std::cout << "Initial pose mode: " << initial_pose_mode << std::endl;
  // std::cout << "Pedal on: " << pedal_on << std::endl;
...
void MiniEffortNode::update()
{
  ...
  tool0_T_ee(1,1) = 0.0;
  tool0_T_ee(1,2) = 0.0;
  tool0_T_ee(1,3) = mini_fk_xz(0);
  tool0_T_ee(2,0) = 0.0;
  tool0_T_ee(2,1) = 0.0;
  tool0_T_ee(2,2) = 1.0;
  tool0_T_ee(2,3) = mini_fk_xz(1);
  tool0_T_ee(3,0) = 0.0;
  tool0_T_ee(3,1) = 0.0;
  tool0_T_ee(3,2) = 0.0;
  tool0_T_ee(3,3) = 1.0;
  ...
  error_iiwa(0)= d_pos0_(0) - x ;
  error_iiwa(1)= d_pos0_(1) - z ;
  ...
  if (!pedal_on){
    tool0_T_ee_mini_desired = world_T_tool0.inverse() * world_T_ee;
    d_pos_(0) = tool0_T_ee_mini_desired(1,3);
    d_pos_(1) = tool0_T_ee_mini_desired(2,3);
