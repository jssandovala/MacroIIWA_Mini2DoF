...
//   ee_wrt_world_pub = this->create_publisher<std_msgs::msg::Float64MultiArray>(
//       "/ee_wrt_world", 10);
...
void MiniEffortNode::iiwa_fk_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
    ...
    //For graphs
    // auto ee_wrt_world_msg = std::make_unique<std_msgs::msg::Float64MultiArray>();
    // ee_wrt_world_msg->data.resize(4);
    // ee_wrt_world_msg->data[0] = world_T_ee_IM(0, 3); // X
    // ee_wrt_world_msg->data[1] = world_T_ee_IM(1, 3); // Y
    // ee_wrt_world_msg->data[2] = world_T_ee_IM(2, 3); // Z
    // ee_wrt_world_msg->data[3] = this->get_clock()->now().seconds();  // timestamp
    // ee_wrt_world_pub->publish(std::move(ee_wrt_world_msg));

    // For debugging
    // std::cout << std::fixed << std::setprecision(3);
    // std::cout << "Pose ee in world frame: "
    //           << world_T_ee_IM(0, 3) << ", "
    //           << world_T_ee_IM(1, 3) << ", "
    //           << world_T_ee_IM(2, 3) << std::endl;
